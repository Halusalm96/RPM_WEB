...
if (isset($_GET['target_key'])) {
    ...
    if ($newCode !== false) {
        ...
        if (updateManagerCode($targetKey, $newCode)) {
    ...
    // target_key가 제공되지 않은 경우에 대한 오류 처리
    $response['success'] = false;
    $response['error'] = "target_key가 제공되지 않았습니다.";
}
...
function updateManagerCode($targetKey, $newCode) {
    global $conn;

    // 데이터베이스 업데이트를 위한 SQL 쿼리를 준비합니다.
    $sql = "UPDATE target SET manager_code = ? WHERE target_key = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("si", $newCode, $targetKey);
    $result = $stmt->execute();
    $stmt->close();
