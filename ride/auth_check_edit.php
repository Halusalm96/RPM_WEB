...
function checkUserRole($targetKey) {
    if (isset($_SESSION['loggedin']) && $_SESSION['loggedin'] === true) {
        ...
        if ($role === '총관리자') {
            ...
            $sql = "SELECT manager_code FROM target WHERE target_key = ?";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("i", $targetKey);
            $stmt->execute();
            $stmt->store_result();
            ...
            if ($stmt->num_rows > 0) {
                ...
                // 사용자가 입력한 관리자 코드
                $enteredCode = $_SESSION['entered_manager_code'][$targetKey];
